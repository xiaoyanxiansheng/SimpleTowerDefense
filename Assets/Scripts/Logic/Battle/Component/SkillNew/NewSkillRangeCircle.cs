...
{
    private EntityBase _bullet = null;

    public override void InitSkill(int skillId, int skillLevel, int skillIndex)
    {
        base.InitSkill(skillId, skillLevel, skillIndex);
        _bullet = GameApp.Instance.entityManager.CreateEntity<EntityBase>(EntityType.SKill, GetSkillConfig().entityId);
    }
    ...
    {
        EntityBase followEntity = GameApp.Instance.entityManager.GetEntityByDistance(EntityType.Enemy, GetPosition(), GetDistance());
